...
If you want to skip ahead you can simply use the supplied link:../osbuild/webserver.toml[webserver.toml],
which can be deployed as follows if you've got a local copy of the git repo.

[source,bash]
----
composer-cli blueprints push /opt/ephemeral-world/osbuild/webserver.toml
----
...
customise our link:../osbuild/webserver.toml[webserver.toml] further to start the service and enable
